...
Pin a specific version: append `-s v1.0.3`. Override the install location with `TWELVEDATA_INSTALL=<dir>` (default `~/.twelvedata`).
...
Pin a specific version: `$env:TWELVEDATA_VERSION = 'v1.0.3'` before piping. Override the install location with `$env:TWELVEDATA_INSTALL`.

### Go

```sh
go install github.com/twelvedata/twelvedata-cli/cmd/twelvedata@v1.0.3
```
...
```json
{
  "ok": true,
  "checks": [
    { "name": "CLI Version",        "status": "pass", "message": "v1.0.3 (latest)" },
...
```
A new version of twelvedata is available: v1.0.3 → v<latest>
